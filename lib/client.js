...
function Iterator() {
	this.it = null; //iterator uid
	this.client = null;
}

var iterator = new Iterator();

Iterator.prototype.seek = function(key, cb) {
	if(this.it) {
		//console.log('calling iterator.seek(' + key + ')');
		this.client.rpc('iterator_seek', {uid: this.it, key: key}, cb);
	}
};

Iterator.prototype.next = function(cb) {
	if(this.it) {
		//console.log('calling iterator.next(cb)');
		this.client.rpc('iterator_next', {uid: this.it}, cb);
	}
};

Iterator.prototype.end = function(cb) {
	if(this.it) {
		//console.log('calling iterator.end');
		this.client.rpc('iterator_next', {uid: this.it}, cb);
	}
};

Db.prototype.iterator = function(options, cb) {
	//console.log('calling iterator');
	var theClient = this.client;
	this.client.rpc('iterator', options, function(err, result) {

		//console.log('iterator cb: error=' + err + ',result=' + result);
		if(err) {
			cb(err, null);
		}
		else {
