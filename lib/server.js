...
return function (db, opts) {
  ...
  server.createLocalCall('iterator', function (args, cb) {
	...
	//console.log('returning iterator'+ iterator);
	cb(null, iterator);
  });
  
  server.createLocalCall('iterator_seek', function (args, cb) {
    //console.log('calling server iterator.seek with options:' + args);
	iterator.seek(args['key']);
	cb(null, iterator);
  });
  
  server.createLocalCall('iterator_next', function (args, cb) {
    //console.log('calling server iterator.next with options:' + args);
  ...
  server.createLocalCall('iterator_end', function (args, cb) {
    console.log('calling server iterator.end with options:' + args);
	iterator.end();
	cb(null, null);
  });
