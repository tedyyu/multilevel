...
return function (db, opts) {
  ...
  //start: add feature for iterator by tedy yu
  var iterators = {};
  
  server.createLocalCall('iterator', function (args, cb) {
    //console.log('calling server iterator with options:' + JSON.stringify(args) + ',cb=' + cb);
	iterator = db.iterator(args);
	var uid = uuid.v1();
	while (iterators[uid]) uid = uuid.v1();
	iterators[uid] = iterator;
	//console.log('returning iterator with uid:' + uid);
	cb(null, uid);
  });
  
  server.createLocalCall('iterator_seek', function (args, cb) {
	//console.log('calling server iterator.seek with options:' + JSON.stringify(args) + ' -> iterator uid:' + args['uid']);
	var iterator = iterators[args['uid']];
	iterator.seek(args['key']);
	cb(null, iterator);
  });
  
  server.createLocalCall('iterator_next', function (args, cb) {
	//console.log('calling server iterator.next with options:' + JSON.stringify(args) + ' -> iterator uid:' + args['uid']);
	var iterator = iterators[args['uid']];
  ...
  server.createLocalCall('iterator_end', function (args, cb) {
	//console.log('calling server iterator.end with options:' + JSON.stringify(args) + ' -> iterator uid:' + args['uid']);
	var iterator = iterators[args['uid']];
	iterator.end(cb);
  });

  //end: add feature for iterator by tedy yu
